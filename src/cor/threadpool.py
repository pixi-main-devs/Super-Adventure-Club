...
class ThreadPool:
    ...
    """A thread pool is a construct used to simply the use of threads 
    and decrease the overhead of starting up worker threads for short-lived 
    tasks. 

    When the pool is created, it creates a number of worker threads, each 
    of which waits for work to do. 

    When the pool is given work, it dispatches the work to one of the waiting 
    threads, which does the work, returns the result, and then goes back to 
    waiting for more work. 

    The pool can be dynamically resized (for example, based on feedback like 
    the number of threads currently idle, or the number of unexecuted tasks 
    in the queue)."""
